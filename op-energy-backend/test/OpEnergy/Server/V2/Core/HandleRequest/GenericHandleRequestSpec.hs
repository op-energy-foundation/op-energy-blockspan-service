...
spec :: Spec
spec = do
  describe "handleRequest-generic" <! do
    tip <- runIO <! QC.generate <! QC.choose (0,500)
    blockchain <- runIO <! Bitcoin.generateBlockChain Bitcoin.genesisMediantime 0
      (verifyNatural tip)
    (envState, env) <- runIO <! Env.init "handleRequest-generic" blockchain
    let
    ...
    it "Scheduler.iteration should not fail" <! do
      shouldContinue <- runAppM "test" env <! do
        Scheduler.iteration
